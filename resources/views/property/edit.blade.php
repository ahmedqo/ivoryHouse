...
@section('title', 'تعديل العقار ' . $data->id)
    ...
    <div class="w-full bg-gray-950 rounded-lg lg:rounded-2xl -mt-12 overflow-hidden">
        <div class="p-4 flex items-center justify-between gap-4">
            <h1 class="font-black text-gray-50 text-xl">
                تعديل العقار {{ $data->id }}
            </h1>
            <div class="w-max flex items-center gap-2">
                <a href="{{ route('actions.properties.destroy', $data->id) }}"
    ...
    <div class="w-full bg-gray-950 p-4 rounded-lg lg:rounded-2xl">
        <form action="{{ route('actions.properties.edit', $data->id) }}" method="POST" enctype="multipart/form-data"
                ...
                <div class="w-full flex flex-col gap-2 lg:col-span-2">
                    ...
                    <div class="relative">
                        <input oninput="makeSlug(event)"
                            ...
                            type="text" id="title" name="title" value="{{ $data->title }}" />
                        <input type="text" name="slug" class="hidden" value="{{ $data->slug }}" />
                    </div>
                </div>
            ...
            <div class="grid grid-rows-1 grid-cols-1 lg:grid-cols-2 gap-4">
                <div class="w-full flex flex-col gap-2">
                    <label for="area" class="text-gray-50 text-md font-black">المساحة</label>
                    <input
                        class="appearance-none bg-[#4c4643] text-gray-50 h-[48px] text-lg rounded-md lg:rounded-xl block w-full py-2 px-4"
                        id="area" name="area" type="number" value="{{ $data->area }}" />
                </div>
                <div class="w-full flex flex-col gap-2">
                    <label for="rooms" class="text-gray-50 text-md font-black">الغرف</label>
                    <input
                        class="appearance-none bg-[#4c4643] text-gray-50 h-[48px] text-lg rounded-md lg:rounded-xl block w-full py-2 px-4"
                        id="rooms" name="rooms" type="number" value="{{ $data->rooms }}" />
                </div>
            </div>
            <div class="grid grid-rows-1 grid-cols-1 lg:grid-cols-3 gap-4">
                <div class="w-full flex flex-col gap-2">
                    <label for="kitchen" class="text-gray-50 text-md font-black">المطبخ</label>
                    <div class="relative bg-[#4c4643] text-gray-50 rounded-md lg:rounded-xl">
                        <select x-select id="kitchen" name="kitchen">
                            <option value="1" @if ($data->kitchen) selected @endif>نعم</option>
                            <option value="0" @if (!$data->kitchen) selected @endif>لا</option>
                        </select>
                    </div>
                </div>
                <div class="w-full flex flex-col gap-2">
                    <label for="garage" class="text-gray-50 text-md font-black">الكراج</label>
                    <div class="relative bg-[#4c4643] text-gray-50 rounded-md lg:rounded-xl">
                        <select x-select id="garage" name="garage">
                            <option value="1" @if ($data->garage) selected @endif>نعم</option>
                            <option value="0" @if (!$data->garage) selected @endif>لا</option>
                        </select>
                    </div>
                </div>
                <div class="w-full flex flex-col gap-2">
                    <label for="garden" class="text-gray-50 text-md font-black">الحديقة</label>
                    <div class="relative bg-[#4c4643] text-gray-50 rounded-md lg:rounded-xl">
                        <select x-select id="garden" name="garden">
                            <option value="1" @if ($data->garden) selected @endif>نعم</option>
                            <option value="0" @if (!$data->garden) selected @endif>لا</option>
                        </select>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-rows-1 grid-cols-1 lg:grid-cols-3 gap-4">
                ...
                <div class="w-full flex flex-col gap-2">
                    <label for="map" class="text-gray-50 text-md font-black">خرائط جوجل</label>
                    <input
                        class="appearance-none bg-[#4c4643] text-gray-50 h-[48px] text-lg rounded-md lg:rounded-xl block w-full py-2 px-4"
            ...
            <div class="grid grid-rows-1 grid-cols-1 lg:grid-cols-3 gap-4">
                <div class="w-full flex flex-col gap-2">
                    ...
                    <input
                        class="appearance-none bg-[#4c4643] text-gray-50 h-[48px] text-lg rounded-md lg:rounded-xl block w-full py-2 px-4"
                        id="state" name="state" type="text" value="{{ $data->state }}" />
                </div>
                <div class="w-full flex flex-col gap-2">
                    <label for="city" class="text-gray-50 text-md font-black">المدينة</label>
                    <input
                        class="appearance-none bg-[#4c4643] text-gray-50 h-[48px] text-lg rounded-md lg:rounded-xl block w-full py-2 px-4"
                        id="city" name="city" type="text" value="{{ $data->city }}" />
                </div>
                <div class="w-full flex flex-col gap-2">
                    <label for="zipcode" class="text-gray-50 text-md font-black">الرمز البريدي</label>
                    <input
                        class="appearance-none bg-[#4c4643] text-gray-50 h-[48px] text-lg rounded-md lg:rounded-xl block w-full py-2 px-4"
            ...
            <div class="w-full flex flex-col gap-2">
                ...
                <div id="imagesDisplay"
                    class="w-full bg-[#4c4643] p-4 rounded-lg lg:rounded-2xl grid grid-cols-3 grid-rows-1 lg:grid-cols-10 gap-4">
                    <div
                        class="w-full aspect-square bg-gray-50 bg-opacity-10 hover:bg-opacity-20 transition-opacity rounded-md lg:rounded-xl flex items-center justify-center relative">
                    ...
                    @foreach ($images as $image)
                        <div onclick="deleteImg(event, {{ $image->id }})"
                            ...
                            <img src="{{ asset('storage/files/' . $image->name) }}"
                                class="w-full h-full object-cover pointer-events-none transition-transform group-hover:scale-150" />
                            <div
...
@section('script')
    <script>
        ...
        const deleteImg = function(event, id) {
            const link = "{{ route('actions.files.destroy', ':id') }}".replace(/:id/g, id);
            event.target.remove();
            fetch(link);
        }
